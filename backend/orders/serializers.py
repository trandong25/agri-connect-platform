...
class CartItemSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if affiliate_code:
            if product is None:
                raise serializers.ValidationError({"product": "Bạn phải chọn sản phẩm."})

            affiliate_link = AffiliateLink.objects.filter(code=affiliate_code, product=product).first()
...
class CartSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        data = super().to_representation(instance)
        total_amount = sum((item.product.price * item.quantity for item in instance.items.all()), Decimal("0.00"))
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderItem
        fields = [
            "id", "product", "affiliate_code", "product_name", "unit_name", "unit_price", "quantity", "subtotal",
            "created_date"
        ]
        read_only_fields = fields
...
class SellerOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SellerOrder
        fields = [
            "id", "farmer", "subtotal", "shipping_fee", "discount_amount", "total_amount", "status", "note",
            "confirmed_at", "shipped_at", "completed_at", "items", "status_logs", "created_date", "updated_date"
        ]
        read_only_fields = fields
...
class FarmerSellerOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SellerOrder
        fields = [
            "id", "order", "order_code", "recipient_name", "phone_number", "province", "ward", "address_detail",
            "consumer_note", "subtotal", "shipping_fee", "discount_amount", "total_amount", "status", "note",
            "confirmed_at", "shipped_at", "completed_at", "items", "status_logs", "created_date", "updated_date"
        ]
        read_only_fields = fields
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    cart_item_ids = serializers.ListField(
        child=serializers.IntegerField(min_value=1), write_only=True, allow_empty=False
    )
    ...
    class Meta:
        model = Order
        fields = [
            "id", "code", "address", "cart_item_ids", "payment_method", "recipient_name", "phone_number", "province",
            "ward", "address_detail", "subtotal", "shipping_fee", "discount_amount", "total_amount", "note",
            "seller_orders", "created_date", "updated_date"
        ]
        read_only_fields = [
            "id", "code", "recipient_name", "phone_number", "province", "ward", "address_detail", "subtotal",
            "shipping_fee", "discount_amount", "total_amount", "seller_orders", "created_date", "updated_date"
        ]
    ...
    def create(self, validated_data):
        ...
        products = Product.objects.select_for_update().select_related("farmer", "unit").in_bulk(product_ids)
        seller_data = {}
        ...
        for cart_item in cart_items:
            ...
            if quantity < product.minimum_order_quantity:
                raise serializers.ValidationError({
                    "cart_item_ids": (
                        f"Số lượng tối thiểu của {product.name} là {product.minimum_order_quantity}."
                    )
                })

            if quantity > product.stock_quantity:
                raise serializers.ValidationError({
                    "cart_item_ids": f"Sản phẩm {product.name} hiện chỉ còn {product.stock_quantity}."
                })

            item_subtotal = product.price * quantity
            order_subtotal += item_subtotal
            farmer_data = seller_data.setdefault(product.farmer_id, {
                "farmer": product.farmer, "subtotal": Decimal("0.00"), "items": []
            })
            farmer_data["subtotal"] += item_subtotal
            farmer_data["items"].append({
                "cart_item": cart_item, "product": product, "quantity": quantity, "subtotal": item_subtotal
            })

        order = Order.objects.create(
            consumer=request.user, recipient_name=address.recipient_name, phone_number=address.phone_number,
            province=address.province, ward=address.ward, address_detail=address.address_detail,
            subtotal=order_subtotal, shipping_fee=Decimal("0.00"), discount_amount=Decimal("0.00"),
            total_amount=order_subtotal, note=note
        )

        for farmer_data in seller_data.values():
            seller_subtotal = farmer_data["subtotal"]
            seller_order = SellerOrder.objects.create(
                order=order, farmer=farmer_data["farmer"], subtotal=seller_subtotal,
                shipping_fee=Decimal("0.00"), discount_amount=Decimal("0.00"), total_amount=seller_subtotal
            )
            SellerOrderStatusLog.objects.create(
                seller_order=seller_order, old_status="", new_status="PENDING", changed_by=request.user
            )

            for item_data in farmer_data["items"]:
                product = item_data["product"]
                OrderItem.objects.create(
                    seller_order=seller_order, product=product,
                    affiliate_link=item_data["cart_item"].affiliate_link, product_name=product.name,
                    unit_name=product.unit.name, unit_price=product.price, quantity=item_data["quantity"],
                    subtotal=item_data["subtotal"]
                )
                product.stock_quantity -= item_data["quantity"]
                product.save(update_fields=["stock_quantity", "updated_date"])

        Payment.objects.create(order=order, method=payment_method, amount=order.total_amount, status="PENDING")
